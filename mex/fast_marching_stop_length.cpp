...
#define ID_(i,j,k) ACCESS_ARRAY(ID,i,j,k)
					...
					int nrhs, const mxArray*prhs[] )
{
	...
	if( nlhs>=2 )
	{
		...
		ID = (unsigned char *) mxGetData(plhs[2]);
		
		mwSize  dims2[2] = {nb_start_points,3};
		plhs[3] = mxCreateNumericArray(2, dims2 ,  mxDOUBLE_CLASS, mxREAL );
		stop_tracking = (double *) mxGetData(plhs[3]);
...
//        mexPrintf("\n B(0,0,0) is: %d \n",B_(0,0,0));
	...
	bool* Max_Length_Reached = new bool[nb_start_points];
	...
	int percentage = 10;
	int num_iter = 0;
	double current_length,current_min;
	unsigned char current_ID=100;
	...
	//	mexPrintf("\n B(%d,%d,%d) is: %d",i,j,k,B_(i,j,k));
        if( B_(i,j,k)==1  )
        {
						...
						if(nei_i2!=0 || nei_j2!=0 || nei_k2!=0){
							...
							if( ii>=0 && jj>=0 && ii<n && jj<p && kk>=0 && kk<q  )
							{
								if (D_(ii,jj,kk)<current_min && S_(ii,jj,kk)==kDead && D_(ii,jj,kk)!=GW_INFINITE){
									...
									current_ID = ID_(ii,jj,kk);
									//mexPrintf("\n ENTERING: %d",current_ID);
								}
							}
						}
					}
			...
			if (current_ID!=100)
				ID_(i,j,k)=current_ID;
				
			//mexPrintf("\n PASSING: %d",current_ID);
			//	mexPrintf("\n C_: %f",C_(i,j,k));
			if (C_(i,j,k)>max_length){
				//mexPrintf("\n PASSING: %d",current_ID);
				if (Max_Length_Reached[current_ID]==false){
					stop_tracking[current_ID+nb_start_points*0] = (double) i+1;
					stop_tracking[current_ID+nb_start_points*1] = (double) j+1;
					stop_tracking[current_ID+nb_start_points*2] = (double) k+1;
					//mexPrintf("\n Stop_: %d %d %d",i,j,k);
				}
				Max_Length_Reached[current_ID]=true;
				stop_iteration = true;
				for (int s=0;s<nb_start_points;s++){
					if (Max_Length_Reached[s]==false){
						stop_iteration = false; break;
					}					
				}
				D_(i,j,k)=max_length;	
			}
